...
在变量字段之后，属性字段是第二个用来处理类中的数据的选项。不像变量字段，属性字段能够对字段的访问规则和生成规则提供更多控制权，通常有以下使用场景：
...
- 一个可以被从任何地方读取，但是只能从定义所在的类内部进行写入的字段
...
>
> **定义** 写入访问：
> 一个字段的写入访问发生于使用字段访问表达式进行赋值时，比如 `obj.field = value` 。当使用 `+=` 等特殊的赋值操作符时，读取访问也会同时发生，比如表达式 `obj.field += value`。
>
...
声明的语法大体上和变量字段的语法相似，事实上他们的语法规则是一样的。但是属性字段的声明有以下特征：
...
属性字段的出现会在类型系统上产生一些影响。其中最重要的是要知道属性字段是一个编译时特性，因此字段的类型声明在编译时必须是已知的。如果我们将一个带有属性字段的类赋值到 `Dynamic` 对象上，其中的字段访问将不再遵循存取器方法。同样，访问限制也将不再适用，此时所有的访问几乎都是 public 的。
...
class Main {
	static public function main() {
		...
		// 因为字段的读取访问行为是 null 所以下面这行代码会产生编译时错误
		// trace(new Test().propertie);
        
         // 赋值到 Dynamic 对象之后属性字段的访问限制不再生效
...
`dynamic` 访问修饰符的作用与 `get` 或 `set` 一样，但是编译器不会检查存取器是否存在。
